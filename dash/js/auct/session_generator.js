	...
	firebase.database().ref('/sessions/').once('value').then(function(snapshot) {
	  ...
	  for (var i = 0; i < dbSessions.length; i++) {
	  	if(dbSessions[i] != null) {
	  				// The session class is created and instantiated with DB fields
	  		  	var s = {
	  		  		id : i,
	  		  	...
	  		  	s.markup = "<div class='panel panel-"+type+"'>"
	  		  						+"<div class='panel-heading'>"
	  									+"<div class='panel-title-box'>"
	  									+"<h3>"+s.language+" | "+s.date+" | list"
	  									+s.wordlistref+"</h3></div></div>"
	  									+"<div class='panel-body'>"
	  									+"<form class='form-horizontal' role='form'>"
	  									+"<div class='form-group'>"
	  									+"<div class='col-md-12 segment-holder'>"
	  									+"<button class='btn btn-info btn-block seg-load-button' "
	  									+"type='button'"
	  									+"onclick='loadSeg("+s.id+","+s.wordlistref+",this)'"
